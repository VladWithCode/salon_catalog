class ImageSelectorManager {
    ...
    setupEventListeners() {
        ...
        window.eventManager.register('afterSwap', '#images-selector-grid', (event, context) => {
        ...
        this.addClickListener('[data-confirm-selection]', () => this.confirmSelection());
        this.addClickListener('[data-clear-selection]', () => this.clearSelection());
        this.addClickListener('[data-switch-tab]', (event) => {
            const tab = event.target.getAttribute('data-tab');
            this.switchTab(tab);
        });
        this.addClickListener('[data-image-item]', (event) => {
            this.toggleImageSelection(event.target.closest('[data-image-item]'));
        });
    ...
    loadPreselectedImages() {
        if (this.config.selectedIds && this.config.selectedIds.length > 0) {
            this.config.selectedIds.forEach(id => {
                const element = this.modalElement.querySelector(`[data-image-id="${id}"]`);
                if (element) {
    ...
    handleGridUpdate(event, context) {
        ...
        this.selectedImages.forEach((imageData, imageId) => {
            const element = this.modalElement.querySelector(`[data-image-id="${imageId}"]`);
            if (element) {
    ...
    switchTab(tab) {
        const selectTab = this.modalElement.querySelector('[data-tab="select"]');
        const uploadTab = this.modalElement.querySelector('[data-tab="upload"]');
        const selectContent = this.modalElement.querySelector('[data-content="select"]');
        const uploadContent = this.modalElement.querySelector('[data-content="upload"]');
        
        if (tab === 'select') {
            selectTab?.classList.add('border-accent', 'bg-gray-50');
            selectTab?.classList.remove('border-transparent', 'text-gray-500');
            uploadTab?.classList.remove('border-accent', 'bg-gray-50');
            uploadTab?.classList.add('border-transparent', 'text-gray-500');
            
            selectContent?.classList.remove('hidden');
            uploadContent?.classList.add('hidden');
        } else if (tab === 'upload') {
            uploadTab?.classList.add('border-accent', 'bg-gray-50');
            uploadTab?.classList.remove('border-transparent', 'text-gray-500');
            selectTab?.classList.remove('border-accent', 'bg-gray-50');
        ...
        if (isSelected) {
            ...
            if (this.config.mode === 'multiple') {
                if (this.selectedImages.size >= this.config.maxSelection) {
                    this.showMessage(`Máximo ${this.config.maxSelection} imágenes permitidas`, 'warning');
                    return;
                }
            } else {
                this.clearSelection();
    ...
    applySelectionToElement(element, imageId) {
        ...
        if (indicator) {
            indicator.classList.remove('bg-white', 'border-gray-300', 'group-hover:border-gray-400');
    ...
    deselectImageElement(element, imageId) {
        ...
        const indicator = element.querySelector('[data-selection-indicator]');
        if (indicator) {
            indicator.classList.add('bg-white', 'border-gray-300', 'group-hover:border-gray-400');
    ...
    updateSelectionUI() {
        ...
        const confirmButton = this.modalElement.querySelector('[data-confirm-selection]');
        ...
        if (this.config.mode === 'multiple') {
            const countElement = this.modalElement.querySelector('[data-selected-count]');
            if (countElement) {
    ...
    updateSelectedImagesPreview() {
        const previewContainer = this.modalElement.querySelector('[data-selected-preview]');
        ...
        this.selectedImages.forEach((imageData, imageId) => {
            ...
            const removeBtn = preview.querySelector('[data-remove-from-selection]');
            removeBtn.addEventListener('click', () => this.removeFromSelection(imageId));
            this.eventListeners.push({ element: removeBtn, event: 'click', handler: () => this.removeFromSelection(imageId) });
        });
    }
    ...
    loadPage(page) {
        const searchInput = this.modalElement.querySelector('[data-search-input]');
        const sortSelect = this.modalElement.querySelector('[data-sort-select]');
    ...
    loadUploadForm() {
        const container = this.modalElement.querySelector('[data-upload-container]');
        if (container && container.innerHTML.trim() === '') {
            if (window.htmx) {
                window.htmx.ajax('GET', '/imagenes/subir', {
                    target: '[data-upload-container]',
                    swap: 'innerHTML'
                });
            }
        }
    ...
    async confirmSelection() {
        ...
        if (this.config.updateEndpoint) {
            try {
                ...
                this.showMessage('Imágenes actualizadas correctamente', 'success');
                this.close();
            } catch (error) {
    ...
    updateFormField(selectedIds) {
        ...
        this.updateFormImagePreview(targetField, Array.from(this.selectedImages.values()));
    }
