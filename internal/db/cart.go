...
func (c *Cart) LoadItems(ctx context.Context) error {
	...
	rows, err := conn.Query(ctx, `
		SELECT ci.product_id, ci.quantity, ci.source, ci.created_at, ci.updated_at,
		       cp.name, COALESCE(cp.category_name, ''), cp.image_url, cp.quantity as max_quantity, cp.available
		FROM cart_items ci
	...
	for rows.Next() {
		...
		err = rows.Scan(
			...
			&item.Name, &item.Category, &item.ImageURL, &item.MaxQty, &item.Available,
		)
		if err != nil {
			return err
		}
