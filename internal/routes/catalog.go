...
func RegisterCatalogRoutes(router *customServeMux, cartSessions *session.CartManager) {
	router.HandleFunc("GET /catalog/categories", publicMiddleware(cartSessions, GetCatalogCategories))
	router.HandleFunc("GET /catalog/products", publicMiddleware(cartSessions, GetCatalogProducts))
	router.HandleFunc("GET /catalogo/producto/{id}", publicMiddleware(cartSessions, GetProductDetail))

	// Compat for old printed QR codes
	router.HandleFunc("GET /productos/{id}", publicMiddleware(cartSessions, GetProductDetail))
}
...
// catalogProductDetailLoader and relatedProductsLoader are the narrow
// dependencies GetProductDetail needs from internal/db. They exist so
// tests can substitute fakes without a database — the same pattern
// internal/routes/cart_forms.go already uses for cartFormMutationService.
// Production always binds these to db.FindCatalogProductDetail and
// db.FindRelatedProducts, the single resolver both /catalogo/producto/{id}
// and /productos/{id} depend on (see catalogProductIdentifierKind in
// internal/db/catalog.go) — this wrapper never re-implements that logic.
type catalogProductDetailLoader func(id string) (*db.CatalogProd, error)
type relatedProductsLoader func(productID string, limit int) ([]*db.CatalogProd, error)

func GetProductDetail(w http.ResponseWriter, r *http.Request) {
	getProductDetailHandler(db.FindCatalogProductDetail, db.FindRelatedProducts)(w, r)
}

func getProductDetailHandler(loadDetail catalogProductDetailLoader, loadRelated relatedProductsLoader) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := r.PathValue("id")
		isAjax := r.Header.Get("HX-Request") == "true"
		modalState := &components.ProductModalState{}
		product, err := loadDetail(id)
		...
		modalState.RelatedProducts, err = loadRelated(product.ID, 6)
		if err != nil {
