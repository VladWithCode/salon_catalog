...
func RegisterCategoriesRoutes(router *customServeMux) {
	...
	// Legacy JSON API routes. GET remains public (read-only listing, same
	// posture as the other public catalog reads). The three mutations were
	// found registered without auth.ValidateAuth — unauthenticated
	// create/update/delete of categories — and are gated here the same way
	// every other admin mutation in this file already is.
	router.HandleFunc("GET /api/categories", GetCategories)
	router.HandleFunc("POST /api/categories", auth.ValidateAuth(CreateCategory))
	router.HandleFunc("PUT /api/categories/{id}", auth.ValidateAuth(UpdateCategory))
	router.HandleFunc("DELETE /api/categories/{id}", auth.ValidateAuth(DeleteCategory))
}
...
func CreateCategory(w http.ResponseWriter, r *http.Request, a *auth.Auth) {
	err := r.ParseForm()
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
...
func UpdateCategory(w http.ResponseWriter, r *http.Request, a *auth.Auth) {
	id := r.PathValue("id")
	err := r.ParseForm()
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
...
func DeleteCategory(w http.ResponseWriter, r *http.Request, a *auth.Auth) {
	id := r.PathValue("id")
	err := db.DeleteCategory(id)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
